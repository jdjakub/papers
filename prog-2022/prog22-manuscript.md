...
To build more concrete intuition behind the notion of a _programming system_, this section discusses a number of examples, classified into three broader types of programming systems. Our classification, loosely inspired by Lehman's classification of _software systems_ \cite{SPEPrograms}, identifies three types of programming systems:

- **L-type programming systems** are software ecosystems built around text-based programming language. They consist of set of more or less integrated tools such as command-line compilers, debuggers and Integrated Development Environments.

- **O-type programming systems** resemble operating systems in that they also structure the execution environment and ignore what exists outside of the system. They provide common interface for communication, both between the user and the computer and between programs.

- **A-type programming systems** are programmable applications. They are often primarily domain-specific and offer a limited degree of programmability, although they often evolve to become more flexible and powerful.

Before presenting the technical dimensions framework, we provide several examples for each of the type of programming systems. This serves three purposes. First, looking at a number of examples from the past helps build an intuitive understanding of what we mean by a programming system. Second, it allows us to introduce example systems that we will use in the next section to illustrate the individual technical dimensions. Third, studying the difference between systems in individual families is one way of identifying and motivating interesting technical dimensions.

## L-type programming systems

We see programming systems as (collections of) software artifacts that make it possible to construct programs, debug them, and turn them into operational, maintained, and evolvable artifacts running on an appropriate hardware. For programming systems built around text-based programming languages, the boundaries of a programming system are not explicitly defined. To speak of a programming system, we need to consider a programming language with, at least, an editor and a compiler. However, the exact delineation is a design choice that significantly affects our analysis.

\paragraph{Java with the Eclipse ecosystem.}
Java cannot be viewed as a programming system on its own, but it is a programming system if  we consider it as embedded in an ecosystem of tools. There are multiple ways of delineating the system, resulting in different analyses. A minimalistic programming system would consist of a text editor to write Java code and a command line compiler. A more realistic programming system is Java as embedded in the Eclipse IDE. The programming systems view allows us to see all there is beyond the textual code. In case of Eclipse, this includes debugger, refactoring tools, testing and modelling tools, GUI designers, and many more. This delineation yields a programming system that is powerful and convenient, but has a large number of concepts and notations that the programmer needs to master.

\paragraph{Haskell tools ecosystem.}
Haskell is an example of an even more language-focused programming system, which is why we use it as an example throughout this paper. It is typically used through a command-line compiler (GHC) and REPL (GHCi) alongside with a text editor that typically provides tools such as syntax highlighting and auto-completion. In case of Haskell, the specific editor is not relevant and our analysis can consider any editor that supports the Language Server Protocol \cite{LSP}.
...
The previously discussed programming systems were either universal, in that they did not focus on any particular kind of application, or they were focused on broad application areas. Lisp, for example, was designed for symbolic data manipulation in the context of Artificial Intelligence, while FORTRAN focused on scientific computing. However, as computers became more widely used, it became clear that there are more narrow typical kinds of applications that need to be built. For those, specialized programming systems began to appear. Although they are more focused, they also support programming based on rich interactions with specialized visual and textual notations.
