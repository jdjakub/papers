...
Like most programming languages, Haskell code can be written in a wide range of text editors, some of which support assistance tools such as syntax highlighting and auto-completion. These offer immediate feedback while editing code, such as when highlighting type errors.
...
One could argue that socialibilty is not purely a technical dimension, as it includes aspects of product management. Rather we believe that sociability is a pervasive cross-cutting concern that can not be separated from the technical.
