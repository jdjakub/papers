...
A *programming system* may include tools, protocols, notations, and languages. It is a software artifact that makes it possible to construct programs, debug them, and turn them into operational, maintained, and evolvable artifacts running on appropriate hardware. This notion covers classic programming languages together with their editors, debuggers, compilers, and other tools. It also covers self-sufficient programming environments like Smalltalk, programming environments centered on operating systems like UNIX, and hypermedia authoring systems like Hypercard.
...
The academic research on programming suffers from this lack of common vocabulary. While we can thoroughly assess programming languages, as soon as we add interaction or graphics into the picture, we get stuck on how the resulting system is vaguely "cool" or "interesting". Moreover, when designing new systems, inspiration is often drawn from the same few sources. These might be influential past systems like Smalltalk, programmable end-user applications like spreadsheets, or motivational illustrations by thinkers like Victor\ \cite{BretVictor}.
...
Finally, we believe that the aforementioned paradigm shift from programming systems to programming languages has hidden many ideas about programming that are worthwhile recovering and developing further \cite{ComplementaryBasic}. Thus our approach is related to the idea of _complementary science_ developed by Chang\ \cite{Chang} in the context of history and philosophy of science. Chang argues that even in disciplines like physics, superseded theories may contain interesting ideas that are worth documenting, even if the theory has been later falsified (often for other reasons) and discarded. In the field of programming where past systems are discarded for reasons other than empirical failure, Chang's _complementary science_ approach seems even more suitable.
...
1. Programmers often think about program details like design or calculations using  pencil and paper or whiteboard even before writing code. This *supplementary medium* has its own feedback loop, even though this is often not automatic.
...
The possible interactions with a programming system are typically structured through different _modes_ in the sense that certain interactions, and the associated feedback loops, are only available in certain modes. For example, when creating a new project, the user may be able to configure the project through a conversational interface (such as `npm init` in the case of modern JavaScript projects). However, such interactions are no longer available once the project is created. Interestingly, the idea of interaction modes goes beyond just programming systems, appearing in software engineering methodologies. In particular, having a separate *implementation* and *maintenance* phase would be an example of two modes.


*Editing vs debugging.* A good example is the distinction between _editing_ and _debugging_ mode. This is more interesting as it structures the feedback loops the system provides. When debugging a program, the user can modify the program state and get (more) immediate feedback on what individual operations do. (In some systems, one can even modify the program itself during debugging.) However, such feedback loops are not available outside of the debugging mode. In other words, _modes_ can be used as a way of grouping and organizing the many interactions with a system.

*Lisp systems* sometimes distinguish between _interpreted_ and _compiled_ mode. The two modes do not differ just in the efficiency of code execution, but also in the interactions they enable. In the interpreted mode, code can be tested interactively and errors may be corrected during the code execution (see _Error response_). In the compiled mode, the program can only be tested as a whole. The same two modes also exist, for example, in some Haskell systems where the REPL uses an interpreter (GHCi) distinct from a compiler (GHC).
...
Notations are closely linked to representation in that the notation may mirror the structures used for program representation. Basman et al. \cite{Externalize} refer to this as a distinction between "dead" (notation) and "live" (representation) forms).
...
\acks
We would like to thank Richard Gabriel for his detailed comments and shepherding. We also thank the committee of the *Pattern Languages of Programming (PLoP)* conference, who enabled such a collaboration. We have additionally benefited from discussions with Antranig Basman, Philip Tchernavskij, Stepehen Kell, and Luke Church.